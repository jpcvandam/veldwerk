...
#sws_t2905_150922194909_T2905.csv
#sws_t2905_150922194831_T2905.csv
t2905 = pd.read_csv(werkmap +'sws_t2905_150922194831_T2905.csv', skiprows=63, index_col=[0], parse_dates = [0], date_parser = dateparser, decimal=',')#, dtype={'Specifieke geleidbaarheid':np.float64}, engine= 'c', encoding='utf-8')


#tijdzones compenseren
meteo=meteo.tz_localize('UTC')
t2905=t2905.tz_localize('CET')
meteo=meteo.tz_convert('Europe/Amsterdam')
t2905=t2905.tz_convert('Europe/Amsterdam')
#print meteo.index, 'meteo goede tijdzone'
#print t2905.index, 'diver goede tijdzone'
#print t2905
luchtdruk = meteo[14] / 9.80638
#print 'luchtdruk', luchtdruk
waterdruk = t2905['Druk[cm]']/1000.0
...
gecompenseerd = waterdruk - luchtdruk #- 67.685429796 #translatie om te ijken op de plop 222
gecompenseerd.dropna(inplace=True)
#print gecompenseerd, 'gecompenseerd'
gecompenseerd.to_csv('t2905_gecompenseerd.csv', index=True, sep=',')
...
plt.xlabel('Tijd')
plt.title('Gemeten waterstand T2905')
pylab.savefig('waterstandsplot_T2905.png')
pylab.close()

#plot geleidbaarheid
plt.figure(); t2905['1:Geleidbaarheid[mS/cm]'].plot(label='Geleidbaarheid'); t2905['Specifieke geleidbaarheid'].plot(label='specifieke geleidbaarheid'); neerslag.plot(secondary_y=True, label='Neerslag'); #2905['EC ondiep (microS/cm)'].plot(kind='scatter');
#plt.figure(); t2905['Specifieke geleidbaarheid'].plot(); 
#plt.figure(); t2905['EC ondiep (microS/cm)'].plot(); 
#plt.figure(); t2905['EC diep (microS/cm)'].plot();
plt.legend(loc='lower center', shadow=True, fontsize='x-large')
ax = pylab.gca()
ax.set_ylabel('$mS/cm$')
ax.right = neerslag.plot(secondary_y=True)
ax.right.set_ylabel('$mm$')
plt.xlabel('Tijd')
plt.title('Geleidbaarheid T2905')
pylab.savefig('ECplot_T2905.png')
pylab.close()
#print t2905['Specifieke geleidbaarheid']

plt.figure(); t2905['Specifieke geleidbaarheid'].plot(label='Specifieke geleidbaarheid'); 
plt.legend(loc='lower center', shadow=True, fontsize='x-large')
ax = pylab.gca()
ax.set_ylabel('$mS/cm$')
...
plt.xlabel('Tijd')
plt.title('Geleidbaarheid T2905')
pylab.savefig('EC_spec_plot_T2905.png')
pylab.close()
...
ax.set_ylabel('$mm$')
plt.xlabel('Tijd')
plt.title('Neerslag T2905')
pylab.savefig('neerslag_T2905.png')
pylab.close()

#plt.figure(); t2905['EC ondiep (microS/cm)'].plot(kind='line'); 
#plt.figure(); t2905['EC diep (microS/cm)'].plot('line');
#plt.legend(loc='lower center', shadow=True, fontsize='x-large')
#ax = pylab.gca()
#ax.set_ylabel('$mS/cm$')
#plt.xlabel('Tijd')
#plt.title('Handmetingen geleidbaarheid T2905')
#pylab.savefig('EChandmetingen_T2905.png')
#pylab.close()
